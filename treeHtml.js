...
<div class="wrap">
  <div class="frame">
    ...
    <section class="card">
      ...
      <div class="note">اللقب «أبو خشبة» أُطلق أصلًا على منصور الباز العراقى البطائحى، لواقعة العصا الخشبية التى استعان بها لإمساك اللصوص، ثم توارثه ذريته حتى صار «خشبة» هو اللقب الشائع لبعض الفروع اليوم.</div>
    </section>
    ...
    <section class="card">
      ...
      <div class="note">«على خشبة» أول من ارتحل من أسيوط إلى سمالوط، منذ ما يزيد على ٢٠٠ سنة، إبان عهد محمد على باشا وأوائل القرن التاسع عشر، ومعه ابنه محمد وعدد من أقاربه.</div>
    </section>

    <section class="card" id="treeSection">
      <h2>شجرة عائلة خشبة بسمالوط — التفاعلية</h2>
    ...
    <section class="card">
      <details class="shrine-list">
        <summary>📍 بيان أضرحة السادة آل أبو خشبة (اضغط للعرض)</summary>
        <div class="shrine">
          <ol>
...
// ---------------- data ----------------
const treeData = {
  name:"على خشبة", tag:"الجد الأكبر — ارتحل من أسيوط إلى سمالوط", gen:1,
  children:[
    { name:"محمد على خشبة", tag:"ابنه", gen:1,
      children:[
        { name:"محمد بن محمد على خشبة", tag:"أنجب ستة أبناء", gen:2,
          children:[
            { name:"محمود خشبة", tag:"من أعيان سمالوط — رئيس محكمة سمالوط", gen:3,
              children:[
                { name:"عبد الباقى محمود خشبة", tag:"من الأعيان — عضو مجلس الأمة", gen:4,
                  children:[
                    ...
                    { name:"نفيسة", tag:"تزوجت ابن عم أبيها محمود خشبة", gen:5, children:[
                ...
                { name:"هانم محمود خشبة", tag:"تزوجت ابن عمها أحمد حسن خشبة", gen:4 },
                { name:"تفيدة محمود خشبة", tag:"تزوجت ابن عمها محمد حسن خشبة", gen:4 }
              ]
            },
            { name:"أحمد خشبة", tag:"من الأعيان — خريج الأزهر، درّس اللغة العربية إبان عهد الملك فؤاد", gen:3,
              children:[
                { name:"عبد الوهاب", gen:4, children:[{name:"سنية", tag:"تزوجت ابن عمها أنس حسن خشبة", gen:5}] },
                { name:"محمود", tag:"تزوج من نفيسة عبد الباقى خشبة", gen:4 }
              ]
            },
            { name:"محمد خشبة", tag:"من الأعيان", gen:3,
              children:[ { name:"زكية", tag:"تزوجت على طنطاوى الشريعى وأنجبت فوزية عبد الباقى خشبة", gen:4 } ]
            },
            { name:"حسن خشبة", tag:"من الأعيان — تزوج من عائلة خود", gen:3,
              children:[
                { name:"أحمد حسن خشبة", gen:4,
                  children:[
                ...
                { name:"زكيه حسن خشبة", gen:4,
                  children:[
                    { name:"سعودى، شمس، عزت", gen:5 },
                    { name:"آمال", tag:"من زوجها محمد الصغير إسماعيل الشيخ", gen:5 }
                  ]
                },
                { name:"نبوية حسن خشبة", tag:"أولادها: هند، فوزية، عبد الدايم خود", gen:4 }
              ]
            },
            { name:"فاطمة خشبة", tag:"تزوجت سيد محمد شعراوى — أولادها: شلقامى، ليثى، حسين، محمود، وجيده، وجيهه", gen:3 },
            { name:"آمنه خشبة", tag:"جدة أحمد موسى لوالدته، ووالد شحاتة والنقراشى", gen:3 }
          ]
        },
        { name:"عبد الرحمن خشبة", tag:"ابن عم محمد — من الأعيان", gen:2,
          children:[
            { name:"طلعت خشبة", tag:"ابنه الوحيد", gen:3,
              children:[
                { name:"زين العابدين", tag:"تزوج من عائلة الشيخ", gen:4, children:[{name:"طلعت، سنيه، ربيع، أزهار، ساميه، عبد الرحمن، عصمت، مديحه", gen:5}] },
                { name:"نعيمة طلعت خشبة", tag:"تزوجت ابن عمها عبد الباقى خشبة", gen:4 }
              ]
            }
          ]
        }
...
// ---------------- layout (HORIZONTAL: generations flow right → left, RTL) ----------------
const NODE_W = 158;
const COL_W = 220, ROW_H2 = 96;
...
// n._row = vertical stacking position (breadth), n._depth = generation depth from root
function assignPos(n, rowStart, depth){
  n._depth = depth;
  ...
  n._row = (n.children[0]._row + n.children[n.children.length-1]._row) / 2;
  return cursor;
}
...
function render(){
  ...
  const totalW = (maxDepth+1) * COL_W + NODE_W + 40;
  const totalH = treeData._w * ROW_H2 + 60;
  ...
  const pathParts = [];
  function colX(depth){ return totalW - 30 - (maxDepth - depth) * COL_W - NODE_W; }

  function walk(n, parent){
    const px = colX(n._depth);           // left edge of this node's box
    const py = n._row * ROW_H2 + 20;      // vertical center of this node's row

    if(parent){
      const ppx = colX(parent._depth);              // parent's left edge
      const ppy = parent._row * ROW_H2 + 20;
      const startX = ppx;                            // parent connects from its left side
      const startY = ppy;
      const endX = px + NODE_W;                      // child connects at its right side
      const endY = py;
      const midX = (startX + endX) / 2;
      pathParts.push('M '+startX+' '+startY+' C '+midX+' '+startY+', '+midX+' '+endY+', '+endX+' '+endY);
    }
...
function getMaxDepth(n){
  if(!n.children || n.children.length===0 || n.collapsed) return n._depth;
...
function fitView(){
  ...
  scale = Math.min(vw/totalW, vh/totalH, 1) * 0.94;
